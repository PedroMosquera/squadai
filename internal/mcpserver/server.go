...
func RunMCPServer(args []string, stdout, stderr io.Writer, version string, runners map[string]ToolRunner) error {
	for _, arg := range args {
		if arg == "-h" || arg == "--help" {
			...
			fmt.Fprintln(stdout)
			fmt.Fprintln(stdout, "This allows any MCP-capable agent (Claude Code, OpenCode, Cursor,")
			fmt.Fprintln(stdout, "Windsurf, VS Code Copilot, Pi) to call SquadAI tools directly:")
			fmt.Fprintln(stdout, "plan, apply, verify, status, context, memory, etc.")
			fmt.Fprintln(stdout)
			fmt.Fprintln(stdout, "'squadai apply' registers this server in every enabled agent")
			fmt.Fprintln(stdout, "automatically. To register manually (Claude Code example):")
