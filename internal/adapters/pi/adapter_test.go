...
func TestAdapter_ID(t *testing.T) {
	a := New()
	if a.ID() != domain.AgentPi {
		t.Errorf("ID() = %q, want %q", a.ID(), domain.AgentPi)
	}
}
...
func TestDetect_BinaryAndConfigExist(t *testing.T) {
	...
	a := NewWithDeps(
		func(name string) (string, error) { return "/usr/local/bin/pi", nil },
		os.Stat,
	)
...
func TestDetect_StatError_ReturnsError(t *testing.T) {
	a := NewWithDeps(
		func(name string) (string, error) { return "/bin/pi", nil },
...
func TestPaths(t *testing.T) {
	...
	// All Pi paths are home-relative with no OS branching;
	// use filepath.Join so separators are correct on every CI platform.
	wantConfigDir := filepath.Join(home, ".pi", "agent")
	...
	tests := []struct {
		...
		{"CommandsDir", a.CommandsDir(home), filepath.Join(wantConfigDir, "prompts")},
		{"SubAgentsDir", a.SubAgentsDir(home), filepath.Join(wantConfigDir, "agents")},
	}
...
func TestProjectPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{"ProjectConfigFile", a.ProjectConfigFile(project), filepath.Join(project, "pi.json")},
		{"ProjectRulesFile", a.ProjectRulesFile(project), filepath.Join(project, "AGENTS.md")},
		{"ProjectAgentsDir", a.ProjectAgentsDir(project), filepath.Join(project, ".pi", "agents")},
		{"ProjectSkillsDir", a.ProjectSkillsDir(project), filepath.Join(project, ".pi", "skills")},
		{"ProjectCommandsDir", a.ProjectCommandsDir(project), filepath.Join(project, ".pi", "prompts")},
	}
...
func TestSupportsComponent_Memory(t *testing.T) {
	...
	if !a.SupportsComponent(domain.ComponentMemory) {
		t.Error("Pi should support memory component")
	}
}
...
func TestSupportsComponent_NewComponents(t *testing.T) {
	...
	for _, c := range components {
		if !a.SupportsComponent(c) {
			t.Errorf("Pi should support component %q", c)
		}
	}
}
...
func TestSupportsComponent_Unknown(t *testing.T) {
	...
	if a.SupportsComponent(domain.ComponentID("nonexistent")) {
		t.Error("Pi should not support unknown components")
	}
}

func TestSupportsComponent_Workflows(t *testing.T) {
	a := New()
	if a.SupportsComponent(domain.ComponentWorkflows) {
		t.Error("Pi should not support workflows component")
	}
}
...
func TestAdapter_SupportsSubAgents(t *testing.T) {
	...
	if !a.SupportsSubAgents() {
		t.Error("Pi should support sub-agents")
	}
}
...
func TestAdapter_SubAgentsDir(t *testing.T) {
	a := New()
	home := "/Users/test"
	want := filepath.Join(home, ".pi", "agent", "agents")
...
func TestAdapter_SupportsWorkflows(t *testing.T) {
	...
	if a.SupportsWorkflows() {
		t.Error("Pi should not support workflows")
	}
}
...
func TestAdapter_WorkflowsDir(t *testing.T) {
	...
	if a.WorkflowsDir("/project") != "" {
		t.Error("WorkflowsDir should be empty for Pi")
	}
}
...
func TestAdapter_PromptsDir(t *testing.T) {
	a := New()
	home := "/Users/test"
	want := filepath.Join(home, ".pi", "agent", "prompts")
	if got := a.PromptsDir(home); got != want {
